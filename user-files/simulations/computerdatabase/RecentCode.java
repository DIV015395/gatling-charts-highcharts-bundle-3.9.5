...
class RecentCode extends Simulation {
    ...
    ScenarioBuilder scn = scenario("WebSocket Load Test")
            .exec(
                    ws("WebSocket Connect")
                            ...
                            .check(
                                    wsAwait
                                            .withStatusCode(101) // Check for the HTTP 101 status code (Upgrade)
                                            .check(regex("HTTP/1.1 101").saveAs("webSocketUpgrade"))
                            )
            )
    ...
    {
        setUp(
                scn.injectOpen(atOnceUsers(10)) // Adjust the number of users as needed
